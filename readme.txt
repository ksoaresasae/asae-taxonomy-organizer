...
* Removed debug "Load Pageview Data" button — chart auto-loads on page load

= 1.3.7 =
* Changed GA4 Pageviews chart from horizontal bar to donut chart matching existing report style

= 1.3.6 =
* Fix: Chart.js was excluded from release zip — build-zip.php "vendor" exclude matched admin/js/vendor/
* Renamed admin/js/vendor/ to admin/js/lib/ to avoid collision with root vendor/ exclude
...
* WordPress constants aren't available during class parsing; replaced with literal values (86400, 604800)
* This was killing the entire plugin silently, causing all charts to render blank
...
* Added "View Full Reports" link at the bottom of the dashboard widget

= 1.2.0 =
* Tags matching the assigned category name are now skipped during processing
* AI prompt updated to not suggest tags that duplicate the category
* New "Remove Redundant Tags" tool on Settings tab for bulk cleanup of existing posts
* Chunked AJAX processing handles 18,000+ posts with progress bar and auto-retry
...
* Report ignored tags has its own "Save Report Settings" button
* API key/model/cost settings are never touched when saving report settings
...
* Default ignored tags now applied before first save (article, AssociationsNow, ASAEcenter, podcast, video)
* Tag cache key now includes hash of ignored tags list, so changes auto-invalidate
